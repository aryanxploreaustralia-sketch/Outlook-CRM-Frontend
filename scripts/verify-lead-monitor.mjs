...
check(page.includes('[&_thead_th]:top-(--spacing-topbar)'), 'and rests below the sticky admin topbar, not under it')
check(page.includes('[&_thead_th]:bg-slate-50 '), 'the header background is fully opaque, so a scrolling row cannot show through it')
check(page.includes('[&_thead_th]:backdrop-blur-none'), 'and blurs nothing, since there is nothing behind it to blur')
check(page.includes('[&_thead_th]:shadow-[0_1px_0_0_var(--color-slate-200)]'), 'its separating rule is a shadow, which travels with the sticky cells')
check(table.includes('border-collapse'), 'because the table collapses borders, so the row\'s own border-b cannot')
